...
const sideQuestEvents = Core.applyCampaignProgress(won, Data.caseDefinitions);
assert.strictEqual(won.world.gameStatus, "active");
assert.strictEqual(sideQuestEvents.length, 0);
won.world.finaleReady = true;
const finale = Core.resolveAction(won, "결말");
assert.strictEqual(finale.changed, true);
assert.strictEqual(won.world.gameStatus, "won");
assert.strictEqual(won.world.campaignPhase, "complete");
...
console.log("Tutorial progression, gated finale, deterministic loss state, terminal action guard, and save normalization: PASS");
