...
(function (Core, Data) {
  ...
  function makeState(id, npc, chainIndex) {
    ...
    return {
      id: "goal-" + id + "-" + safeIndex,
      chainIndex: safeIndex,
      status: "active",
      progress: 0,
      target: Math.max(1, Number(definition.target) || DEFAULT_TARGET),
      organization: npc.faction || null,
      priority: 1,
      actions: Array.isArray(definition.actions) ? definition.actions.slice() : [],
      goalText: definition.text,
  ...
  function ensure(state) {
    Object.keys(state.npcs || {}).forEach(function (id) {
      ...
      const chainIndex = Math.max(0, Math.min(chain.length - 1, Number(goal.chainIndex) || 0));
      const definition = chain[chainIndex];

      goal.id = goal.id || ("goal-" + id + "-" + chainIndex);
      goal.chainIndex = chainIndex;
      ...
      goal.target = Math.max(1, Number(goal.target) || Number(definition.target) || DEFAULT_TARGET);
      goal.organization = goal.organization || npc.faction || null;
      goal.priority = Number.isFinite(Number(goal.priority)) ? Number(goal.priority) : 1;
      ...
      goal.completedAt = goal.completedAt == null ? null : Number(goal.completedAt);
      goal.blockedAt = goal.blockedAt == null ? null : Number(goal.blockedAt);
      goal.blockedReason = goal.blockedReason || null;
  ...
  function organizationPressure(state, npc) {
    ...
    if (!id || !state.world.organizations || !state.world.organizations[id]) return 0;
  ...
  function review(state, id, absoluteMinute) {
    ...
    const minute = Number(absoluteMinute) || Core.getAbsoluteMinute(state);
    goal.lastReviewMinute = minute;
    ...
    if (pressure <= -2) {
      if (goal.status !== "blocked") {
        goal.status = "blocked";
        goal.blockedAt = minute;
        goal.blockedReason = "조직의 우선순위가 현재 목표와 충돌한다.";
        goal.lastReviewReason = goal.blockedReason;
    ...
    if (goal.status === "blocked") {
      ...
      goal.lastReviewReason = "조직 우선순위가 회복됐다.";
      return npc.name + "이(가) 조직의 우선순위 변화에 따라 목표를 재개했다: " + goal.goalText;
    }

    goal.priority = Core.clamp(1 + pressure, 0, 3);
    return null;
  }
  ...
  function replan(state, id, absoluteMinute, reason) {
    ...
    if (chain.length < 2) {
      ...
      goal.lastReviewReason = reason || "재계획 가능한 후속 목표가 없음";
      return npc.name + "이(가) 현재 목표를 포기했다: " + goal.goalText;
    }

    const oldText = goal.goalText;
    const nextIndex = (goal.chainIndex + 1) % chain.length;
    const definition = chain[nextIndex];

    goal.completedGoals.push({
      id: goal.id,
      text: oldText,
      status: "abandoned",
    ...
    goal.completedGoals = goal.completedGoals.slice(-8);
    goal.chainIndex = nextIndex;
    goal.id = "goal-" + id + "-" + nextIndex + "-" + (goal.replanCount + 1);
    goal.status = "active";
    goal.progress = 0;
    goal.target = Math.max(1, Number(definition.target) || DEFAULT_TARGET);
    goal.actions = Array.isArray(definition.actions) ? definition.actions.slice() : [];
    goal.goalText = definition.text;
    goal.priority = 1;
  ...
  function isRelevantRoutine(goal, reason) {
    if (!goal.actions || !goal.actions.length) return true;
    return goal.actions.indexOf(reason) !== -1;
  }
  ...
  function progress(state, id, amount, absoluteMinute, reason) {
    ...
    const delta = Math.max(0, Number(amount) || 0) * multiplier;
    ...
    if (goal.progress < goal.target) return reviewEvent;
    ...
    const chain = chainFor(id, npc);
    if (chain.length > 1) {
      const event = advanceAfterCompletion(state, id, goal.completedAt);
      return (npc.name + "이(가) 개인 목표를 달성했다: " + goal.goalText +
        (reason ? " (" + reason + ")" : "") +
        (event ? " " + event : ""));
    }
  ...
  function advanceAfterCompletion(state, id, absoluteMinute) {
    ...
    goal.chainIndex = nextIndex;
    goal.id = "goal-" + id + "-" + nextIndex + "-next";
    goal.status = "active";
    goal.progress = 0;
    goal.target = Math.max(1, Number(definition.target) || DEFAULT_TARGET);
    goal.actions = Array.isArray(definition.actions) ? definition.actions.slice() : [];
    goal.goalText = definition.text;
    goal.priority = 1;
    goal.completedAt = absoluteMinute;
    goal.blockedAt = null;
    goal.blockedReason = null;
    goal.lastProgressMinute = null;
    goal.lastReviewMinute = absoluteMinute;
  ...
  function status(state, id) {
    ...
    return {
      ...
      replanCount: goal.replanCount,
      completedAt: goal.completedAt,
      completedGoals: goal.completedGoals.slice()
    };
  }
