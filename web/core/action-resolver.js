...
(function (Core, Data) {
  function placeFromText(text) {
    const aliases = [
      ["시장", "market"], ["부두", "riverside"], ["강", "riverside"],
      ["골목", "alley"], ["여관", "alley"], ["기록관", "archive"],
      ["기록", "archive"], ["농촌", "hills"], ["언덕", "hills"]
    ];
    for (let i = 0; i < aliases.length; i += 1) {
      if (text.includes(aliases[i][0])) return aliases[i][1];
    }
    return null;
  }
  ...
  function npcFromText(state, text) {
    return Object.entries(state.npcs).find(function (entry) {
      const id = entry[0];
      const npc = entry[1];
      return text.includes(id) || text.includes(npc.name) || text.includes(npc.name.slice(0, 2));
    });
  }
  ...
  function inspect(state) {
    ...
    if (place === "riverside") {
      return state.world.flags.nightCargo
        ? "젖은 밧줄과 화물 자국 사이로 예정표와 맞지 않는 흔적이 보인다."
        : "젖은 밧줄과 화물 자국, 배에서 흘러나온 물자 냄새가 섞여 있다.";
    }
    if (place === "archive") {
      return state.world.flags.recordInconsistency
        ? "같은 토지 번호가 서로 다른 소유자와 연결되어 있다."
        : "공개 열람실의 장부들은 정돈되어 있다.";
    }
    if (place === "hills") {
      return state.world.grainSupply < 60
        ? "농촌의 출하량이 줄었다. 흉작만으로 설명하기 어려운 분위기다."
        : "수확기 직전의 농촌이다. 운송로와 저장고가 중요해 보인다.";
    }
  ...
  function caseCommand(state, text) {
    ...
    const entry = state.world.cases.find(function (item) {
      return item.id === caseId && item.status === "open";
    });
    if (!entry || !def) return "그 사건은 진행 중인 사건 목록에 없다.";

    if (!match[2]) {
      return def.title + " — " + def.summary + " 선택지: " +
        def.choices.map(function (choice, index) {
          return (index + 1) + ". " + choice.label + " [위험 " + choice.risk + "]";
        }).join(" / ");
    }
  ...
  Core.resolveAction = function (state, text) {
    ...
    if (/^사건목록$|^사건 목록$|^cases$/i.test(input)) {
      Data.ensureCases(state);
      const open = state.world.cases.filter(function (entry) { return entry.status === "open"; });
      const narrative = open.map(function (entry) {
        return entry.id + " — " + Data.caseDefinitions[entry.id].title;
      }).join(" / ") || "현재 진행 중인 사건이 없다.";
      return { narrative: narrative, action: input, changed: false };
    }

    const place = placeFromText(input);
    if (/이동|가자|간다|가 /i.test(input) && place) {
      result = move(state, place);
    ...
    } else if (/대화|말을|묻|설득|협상/.test(input)) {
      result = talk(state, input);
