...
(function (Core) {
  ...
  function ensure(state) {
    const w = state.world;
    if (!w.economy) {
      w.economy = { prices: { grain: 10 }, stock: { grain: 24 }, simulationDay: -1, tradeVolume: 0 };
    }
  ...
  function trade(state, type, amount) {
    ...
    if (type === "buy") {
      ...
      state.world.relations.merchants = Core.clamp(Number(state.world.relations.merchants || 0) + 1, -100, 100);
