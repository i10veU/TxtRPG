...
동적 경제는 현재 곡물을 첫 번째 거래 상품으로 사용한다. 공급·긴장·치안에 따라 가격이 변하고, 시장 재고와 플레이어의 구매·판매가 세계 상태와 상인회 관계에 반영된다. 가격·재고가 임계치를 넘으면 시장 위기 신호가 생성되어 상인회·농촌·노동자·여관망의 관계와 긴장·소문 압력을 변화시키며, `market-crisis` 사건으로 플레이어의 개입을 요구한다. NPC 사이에도 별도 관계망이 존재하며 협력·갈등이 조직 목표 압력과 `npc-dispute` 사건으로 연결된다.

브라우저 smoke test는 실제 Chromium에서 Worker 실행, IndexedDB 저장, 액션 처리, 페이지 새로고침 후 복원을 검증한다.

## 다음 단계

1. 경제를 곡물 외 지역 자원·상점·교역로로 확장
2. 사건 간 인과망을 후속 사건/지역 변화까지 연결
3. NPC 목표를 장기 지역 변화와 경제 활동에 연결
4. IndexedDB 엔티티 분리와 장기 시뮬레이션 스트레스 테스트
5. 최종 게임 루프·튜토리얼·엔딩/장기 플레이 구조 검증
